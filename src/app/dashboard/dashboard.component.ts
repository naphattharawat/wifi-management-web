...
export class DashboardComponent implements OnInit {
  ...
  async getThaiD() {
    try {
      const rs: any = await this.dashboardService.getCountThaid();
      if (rs.ok) {
        this.listThaidD = rs.rows;
      } else {
        this.alertService.error(rs.error);
      }
    } catch (error) {
  ...
  async getTicket() {
    try {
      const rs: any = await this.dashboardService.getCountTicket();
      if (rs.ok) {
        this.listTicket = rs.rows;
      } else {
        this.alertService.error(rs.error);
      }
    } catch (error) {
